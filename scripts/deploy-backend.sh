...
if [ -f "apps/backend/.env.production" ]; then
    ...
    print_success "Using production environment configuration"
else
    print_error "apps/backend/.env.production not found. Run setup-production.sh first."
    exit 1
fi
...
elif curl -f -s "http://localhost:4300/health" > /dev/null; then
...
pm2 show resumate-backend 2>/dev/null || echo "Process not found"

echo ""
print_status "🌐 Backend URLs:"
echo "  API: http://localhost:4300/api"
echo "  Health: http://localhost:4300/health"
echo "  Production: https://resumate.gftrilo.store/api (if nginx configured)"
...
echo "  View logs:    pm2 logs resumate-backend"
echo "  Restart:      pm2 restart resumate-backend"
echo "  Stop:         pm2 stop resumate-backend"
