...
const generateTexString = (resumeData: ResumeData): string => {
    ...
    const contactLinks = [];
    if (contact.email) {
        contactLinks.push(`\\href{mailto:${escapeLatex(contact.email)}}{${escapeLatex(contact.email)}}`);
    }
    if (contact.linkedin) {
        contactLinks.push(`\\href{${escapeLatex(contact.linkedin)}}}{LinkedIn}`);
    }
    if (contact.github) {
        contactLinks.push(`\\href{${escapeLatex(contact.github)}}}{GitHub}`);
    }
    if (contact.website) {
        contactLinks.push(`\\href{${escapeLatex(contact.website)}}}{Portfolio}`);
    }
    const address2 = contactLinks.join(' \\\\ ');
...
\\begin{rSection}{SUMMARY}
    \\parbox{\\textwidth}{${escapeLatex(summary)}}
\\end{rSection}
` : ''}
