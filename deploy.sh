...
if [ "$SKIP_SETUP" = false ] && [ "$FRONTEND_ONLY" = false ] && [ "$BACKEND_ONLY" = false ]; then
    ...
    if "$SCRIPTS_DIR/setup-production.sh"; then
...
if [ "$BACKEND_ONLY" = false ]; then
    ...
    if "$SCRIPTS_DIR/deploy-frontend.sh"; then
...
if [ "$FRONTEND_ONLY" = false ]; then
    ...
    if "$SCRIPTS_DIR/deploy-backend.sh"; then
...
echo "  Production: https://resumate.gftrilo.store (if nginx configured)"

echo ""
print_status "📋 Post-Deployment Commands:"
echo "  View all logs:    pm2 logs"
echo "  Monitor:          pm2 monit"
echo "  Restart all:      pm2 restart resumate-backend resumate-frontend"
echo "  Stop all:         pm2 stop resumate-backend resumate-frontend"
