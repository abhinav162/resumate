...
class ApiClient {
  ...
  async getTailoredResume(id: string): Promise<TailoredResume> {
    const result = await this.request<TailoredResume>(`/tailored-resumes/${id}`);
    if (!result.data) {
  ...
  async createTailoredResume(tailoredData: {
    ...
    const result = await this.request<TailoredResume>('/tailored-resumes', {
      method: 'POST',
  ...
  async updateTailoredResume(id: string, updateData: Partial<TailoredResume>): Promise<TailoredResume> {
    const result = await this.request<TailoredResume>(`/tailored-resumes/${id}`, {
      method: 'PUT',
